CREATE OR REPLACE TRIGGER GEC_CHAIN_SCHEDULE_REQUEST_TA                                                                                                               
   BEFORE INSERT OR UPDATE OR DELETE OF 
         CHAIN_SCHEDULE_REQUEST_ID, 
         STATUS, 
         CREATED_AT, 
         CREATED_BY
   ON GEC_CHAIN_SCHEDULE_REQUEST
   FOR EACH ROW                                                                                                                                          
DECLARE
   v_opCode CHAR(1); 
BEGIN
   v_opCode := CASE WHEN INSERTING THEN 'I'
                    WHEN UPDATING  THEN 'U'
                    WHEN DELETING  THEN 'D'
               END;
   IF v_opCode = 'I' OR v_opCode = 'U'
   THEN
   :new.LAST_UPDATED_AT := sysdate;
   :new.LAST_UPDATED_BY := substr(NVL(SYS_CONTEXT('USERENV','CLIENT_IDENTIFIER'), 
   	   sys_context('USERENV','OS_USER')|| '@' || sys_context('USERENV','HOST')),1,32);
   END IF;

   INSERT INTO GEC_CHAIN_SCHEDULE_REQUEST_AUD(
         CHAIN_SCHEDULE_REQUEST_ID, 
         STATUS, 
         CREATED_AT, 
         CREATED_BY, 
         LAST_UPDATED_BY,
         LAST_UPDATED_AT,
         OP_CODE
      )
   VALUES (
         DECODE(v_opCode, 'D', :OLD.CHAIN_SCHEDULE_REQUEST_ID, :NEW.CHAIN_SCHEDULE_REQUEST_ID), 
         DECODE(v_opCode, 'D', :OLD.STATUS, :NEW.STATUS), 
         DECODE(v_opCode, 'D', :OLD.CREATED_AT, :NEW.CREATED_AT), 
         DECODE(v_opCode, 'D', :OLD.CREATED_BY, :NEW.CREATED_BY), 
         substr(NVL(SYS_CONTEXT('USERENV','CLIENT_IDENTIFIER'), 
                    sys_context('USERENV','OS_USER') || '@' || sys_context('USERENV','HOST')),1,32),
         SYSDATE,
         v_opCode
   );

END GEC_CHAIN_SCHEDULE_REQUEST_TA;
/